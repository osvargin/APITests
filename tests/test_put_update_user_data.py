...
@allure.suite('Проверка обновления пользователя')
@allure.title('Проверяем обновления пользователя с именем и работой')
def test_update_user_with_name_and_job():
    ...
    with allure.step('Проверяем что время обновления = текущему времени'):
        assert updated_date[0:16] == current_date[0:16]


@allure.suite('Проверка обновления пользователя')
@allure.title('Проверяем обновления пользователя только с работой')
...
@allure.suite('Проверка обновления пользователя')
@allure.title('Проверяем обновления пользователя только с именем')
